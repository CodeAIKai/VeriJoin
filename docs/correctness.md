...
4. the exact answer bytes produced by that program; and
5. SHA-256 versions of every question, title, or sentence cell read by evidence, joins, and answer
   operators.
...
`verify_lineage` detects a missing or bytewise-changed source cell.
...
if the program/answer binding verifies and every source-cell digest remains current, replay returns
the same answer. A change to an unbound distractor cannot change that replay.
